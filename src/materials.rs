...
impl Metal {
    pub fn new(albedo: Color, fuzziness: f64) -> Metal {
        Metal { albedo: albedo, fuzziness: fuzziness }
    }
}
...
impl Material for Dielectric {
    fn scatter(&self, r: &Ray, hit: &Hit) -> Option<ScatterResult> {
        ...
        let direction = if let Some(refracted_direction)  = self.refract(&r.unit_direction(), &hit.normal, eta_frac) {
            refracted_direction // refraction case
        } else {
            r.unit_direction().reflect(&hit.normal) // total reflection case
        };
        Some(ScatterResult::new(attenuation, Ray::new(hit.location.clone(), direction)))
    }
}
